...
class BookRepositoryTest {
    ...
    @Test
    void shouldSaveBook() {
        Book bookAdded = new Book(0L, "Вечера на Хуторе близ Диканьки", "Фантастические истории",
                List.of(author), List.of(genre));
        bookAdded = entityManager.persist(bookAdded);
        entityManager.flush();
    ...
    void shouldReturnExpectedListOfBooksByGenre() {
        ...
        List<Book> booksActual = repository.findAllByGenresContaining(genre);
    ...
    void shouldReturnExpectedListOfBooksBtAuthor() {
        ...
        List<Book> booksActual = repository.findAllByAuthorsContaining(author);
    ...
    @Test
    void shouldReturnExpectedListOfBooksByTitle() {
        Book bookAdded = getNewBook();

        List<Book> booksActual = repository.findAllByTitleContaining("на Хуторе");

        assertThat(booksActual).isNotEmpty().hasSize(1)
                .contains(bookAdded)
                .doesNotContain(book);
    }
